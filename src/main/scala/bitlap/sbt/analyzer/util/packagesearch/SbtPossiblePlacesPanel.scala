...
private class SbtPossiblePlacesPanel(
  ...
  private def updateEditor(myCurFileLine: DependencyOrRepositoryPlaceInfo): Unit = {
      ...
      myCurEditor.getDocument.setText {
        if (dep.isDefined) tmpFile.getText
          ...
          SbtDependencyAnalyzerBundle.message(
            "analyzer.packagesearch.dependency.sbt.could.not.generate.expression.string.to.add"
          )
      }
    }
    ...
    scrollingModel.scrollTo(
      new LogicalPosition(math.max(1, oldPos.line - EDITOR_TOP_MARGIN), oldPos.column),
      ScrollType.CENTER
    )
  ...
  private class PlacesCellRenderer extends ColoredListCellRenderer[DependencyOrRepositoryPlaceInfo] {
    ...
    override def customizeCellRenderer(
      list: JList[? <: DependencyOrRepositoryPlaceInfo],
